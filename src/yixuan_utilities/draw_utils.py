...
class Color:
    """
    r: red value in 0 to 1
    g: green value in 0 to 1
    b: blue value in 0 to 1
    a: alpha (transparency) value in 0 to 1 (0 is fully transparent, 1 is fully opaque)
...
def depth2fgpcd_tensor(depth: torch.Tensor, cam_params: torch.Tensor) -> torch.Tensor:
    ...
    pos_x, pos_y = torch.meshgrid(
        torch.arange(w, device=depth.device), torch.arange(h, device=depth.device), indexing="xy"
    )
    pos_x = pos_x.unsqueeze(0).expand(b, -1, -1).reshape(b, h * w)
    pos_y = pos_y.unsqueeze(0).expand(b, -1, -1).reshape(b, h * w)

    cam_params = cam_params.unsqueeze(-1).unsqueeze(-1)  # Expand cam_params to match the shape of pos_x and pos_y
...
def aggr_point_cloud_tensor(
    ...
    pcd_tensor[..., 0] = (pos_x - Ks_tensor[:, 0:1, 2:3]) * depths_tensor / Ks_tensor[:, 0:1, 0:1]
    pcd_tensor[..., 1] = (pos_y - Ks_tensor[:, 1:2, 2:3]) * depths_tensor / Ks_tensor[:, 1:2, 1:2]
    pcd_tensor[..., 2] = depths_tensor

    # map to world frame
    pcd_tensor = pcd_tensor.reshape(N, H * W, 3)
    pcd_tensor = torch.cat(
        [pcd_tensor, torch.ones((N, H * W, 1), dtype=torch.float32, device=device)], dim=-1
    )  # (N, H*W, 4)
    if pose_fmt == ExtriConvention.WORLD_IN_CAM:
        try:
    ...
    if downsample:
        pcd_tensor, pcd_colors = voxel_downsample_torch(pcd_tensor, downsample_r, pcd_colors)
...
def voxel_downsample_torch(
    ...
    voxel_points = torch.zeros(unique_voxels.size(0), points.size(1), dtype=torch.float, device=points.device)

    # Sum points in each voxel
    voxel_points.index_add_(0, indices, points)

    # Count the number of points in each voxel
    counts = torch.zeros(unique_voxels.size(0), 1, dtype=torch.float, device=points.device)
    counts.index_add_(0, indices, torch.ones(points.size(0), 1, device=points.device))

    # Calculate the centroid by dividing sum of points by the number of points in each voxel
    centroids = voxel_points / counts
    if points_color is not None:
        points_color = points_color.float()
        voxel_colors = torch.zeros(unique_voxels.size(0), points_color.size(1), dtype=torch.float, device=points.device)
...
def voxel_downsample_numpy(
    ...
    if points_color is not None:
        ...
        voxel_colors = np.zeros((unique_voxels.shape[0], points_color.shape[1]), dtype=np.float32)
