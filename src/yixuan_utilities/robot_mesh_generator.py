    ...
    def __init__(self, robot_name: str = "trossen_vx300s"):
        ...
        for link in self.urdf_robot.links:
            if len(link.visuals) > 0:
                collision = link.visuals[0]
                if (
                    collision.geometry.mesh is not None
                    and len(collision.geometry.mesh.meshes) > 0
                ):
                    mesh = trimesh.util.concatenate(collision.geometry.mesh.meshes)
                    self.meshes[link.name] = mesh
...
#     kin_helper = RobotMeshGenerator(robot_name=robot_name)
#     for i in range(100):
